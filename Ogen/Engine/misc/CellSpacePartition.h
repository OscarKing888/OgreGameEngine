...
inline void CellSpacePartition<entity>::UpdateEntity(const entity&  ent,
                                                     Vector2D       OldPos)
{
  //if the index for the old pos and the OGRE_NEW pos are not equal then
  ...
  //the entity has moved into another cell so OGRE_DELETE from current cell
  //and add to OGRE_NEW one
