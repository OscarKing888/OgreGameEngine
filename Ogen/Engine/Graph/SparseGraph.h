...
class SparseGraph : public OgeAlloc                                 
{
public:
...
public:
	...
	//clears the graph ready for OGRE_NEW node insertions
...
{
	if ( (m_Nodes[nd].Index() == invalid_node_index) || (nd >= (int)m_Nodes.size()))
	{
		return false;
	}
	else return true;
...
int SparseGraph<node_type, edge_type>::AddNode(node_type node)
{
	...
	{
		//make sure the OGRE_NEW node has been indexed correctly
