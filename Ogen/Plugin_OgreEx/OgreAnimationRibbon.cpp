...
namespace Ogre
{
	...
	AnimationRibbon::AnimationRibbon( unsigned short pointCount, float lifeTime, float ribbonWidth,
	...
	{	
		...
		{
			// create scene node
			Ogre::SceneNode *baseNode = mRootNode;
	...
	void AnimationRibbon::destroyRenderInstance(void)
	{
		...
		if (mBasicNode)
		{
			mRootNode->removeAndDestroyChild(mBasicNode->getName());
			mBasicNode = NULL;
		}	
	...
	//-----------------------------------------------------------------------
	void AnimationRibbon::updateRenderInstance( float time, const Vector3& position, const Quaternion& rot )
	{
		if (mCreateInstance)
		{
			...
			Ogre::Vector3 firstPos = position;

			Ogre::Vector3 dir = rot.xAxis();
			dir.normalise();
			...
			if (mUseInterpolation)
			{
				...
				for (int i=0; i<numLoop; ++i)
				{
					...
					float endOpacity = (1.0f - delta) * mCurrentEndAlphaValue;
					float startOpacity = (1.0f - delta) * mCurrentStartAlphaValue;
