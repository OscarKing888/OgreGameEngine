...
namespace std
{
	...
	*/
	...
	public:
		...
		UTFString( const std::string& str, bool isUTF8Source = true ) {
			_init();
			...
			if(!isUTF8Source)
			{
				...
				mbstowcs(&wbuf[0], str.c_str(), s);

				assign(&wbuf[0], wbuf.size());
			}
			else
			{
				assign( str );
		...
		//! changes the size of the string to \a size, filling in any OGRE_NEW area with \a val
		...
		//! returns the current string in the MBCS native form of std::string
		std::string asMBCSStr() const {
			_load_buffer_WStr();

			size_t s = m_buffer.mWStrBuffer->size() * 2;
			char* strBuffer = OGE_ALLOC_T(char, s + 1);
			memset(strBuffer, 0, s + 1);
			wcstombs(strBuffer,  m_buffer.mWStrBuffer->c_str(), s);
			std::string r(strBuffer, s);
			OGE_FREE(strBuffer);
			return r;
		}		
		//@}
	...
	private:
		...
		///////////////////////////////////////////////////////////////////////
		// Scratch buffer
		//! auto cleans the scratch buffer using the proper OGRE_DELETE for the stored type
		void _cleanBuffer() const {
			if ( m_buffer.mVoidBuffer != 0 ) {
				switch ( m_bufferType ) {
				case bt_string:
					OGRE_DELETE m_buffer.mStrBuffer;
					break;
				case bt_wstring:
					OGRE_DELETE m_buffer.mWStrBuffer;
					break;
				case bt_utf32string:
					OGRE_DELETE m_buffer.mUTF32StrBuffer;
					break;
				case bt_none: // under the worse of circumstances, this is all we can do, and hope it works out
				default:
					//OGRE_DELETE m_buffer.mVoidBuffer;
					// OGRE_DELETE void* is undefined, don't do that
		...
		void _getBufferStr() const {
			if ( m_bufferType != bt_string ) {
				_cleanBuffer();
				m_buffer.mStrBuffer = OGRE_NEW std::string();
		...
		void _getBufferWStr() const {
			if ( m_bufferType != bt_wstring ) {
				_cleanBuffer();
				m_buffer.mWStrBuffer = OGRE_NEW std::wstring();
		...
		void _getBufferUTF32Str() const {
			if ( m_bufferType != bt_utf32string ) {
				_cleanBuffer();
				m_buffer.mUTF32StrBuffer = OGRE_NEW utf32string();
